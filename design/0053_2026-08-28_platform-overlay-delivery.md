...
**Status:** Implemented — S1–S4 merged (#1126 S1, #1152 S2, #1156 S3+S4 incl. the S1-remainder artifact), S5 kind suite shipped (#1169) and GREEN (run 33402474321, 2026-08-31: all checks pass — S5.1–S5.5 plus the gVisor leg S5.6, pod Active with both image volumes under runsc; measured cold-pull resume 13–19s across runs, well inside budget and better than the pre-overlay ~22s figure). The S3 merge gate — Epic 70 US-70.1 (sidecar env-class secret handoff, design 0057) — landed first (#1164).
**Date:** 2026-08-28
**Issue:** #1116
...
- **First-party plugins (added #1469 — the "one executable, one hash" phrasing reads, with the #1416 CA-bundle precedent already applied to the agentd artifact, as: one executable whose per-arch sha256 is pinned and verified, plus digest-pinned NON-executable data).** The artifact carries the platform's opencode plugins under `/plugins` (currently `llmsafespaces-origin.js`, the send_message origin-injection plugin, #1465). Rationale: the `tool.execute.before` hook contract is a harness-internal surface, so its consumer MUST be version-coupled to the binary — a plugin facing a drifted hook surface would silently no-op and send_message's origin would silently degrade to self-declared mode. The CI `origin-plugin-pin` job (pinned binary + plugin + stub MCP asserting the injected key arrives, and does NOT arrive without the plugin) is the drift gate on every harness bump. Plugins are data files consumed by the pinned binary (mounted read-only at `/opencode/plugins`, referenced `file://` from `agent-config.json` via `injectPlatformAgentConfig`, user-staged plugin entries preserved concat+dedup); they are never a second executable.
...
> **Status (2026-08-31):** S1 ✅ (#1126 + artifact/CI remainder in #1156) · S2 ✅ (#1152) · S3+S4 ✅ (#1156 — sequenced behind the US-70.1 gate: Epic 70 #1158/#1164, design 0057, per the live-credential-handoff finding) · S5 suite shipped ✅ (#1169 — `local/s5-overlay-validation.yml`); **the S5 gate is green — flip inventory executable** (worklog 0871/0873 lineage; runbook `docs/runbooks/sidecar-flip.md`).
