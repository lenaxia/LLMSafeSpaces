...
`pkg/secrets/README.md:46-51` is explicit that the **dominant threat is RCE in the API pod** (cross-referenced from `design/stories/epic-17-security-review/THREAT-MODEL.md` attack tree 2.4, lines 141-146). Once an attacker runs code in the pod, they call `Decrypt()` exactly as legitimate code does. No at-rest encryption scheme prevents in-process abuse while the RCE is live — the honest framing from `pkg/secrets/README.md:42-44`.
...
**Three KMS keys, not four.** `apiKeyProv` (P3) is shared by API keys and SSO client secrets today and stays shared under KMS. The SSO service receives `apiKeyProv` as its `KeyProvider` at `api/internal/app/app.go:608`; migrating them separately would require a new config path and a new KMS key with no threat-model benefit.
...
- **D9 — KMS unavailability is a single-point-of-failure availability dependency.** Under KMS, every `Decrypt` call is a network round-trip to the cloud API. Sustained KMS unavailability (regional outage, network partition to the KMS endpoint) causes all KEK-dependent decrypts to fail simultaneously: API-key auth, SSO client secret decrypt, and admin/org credential access all degrade at once. This dependency does not exist under the local static/sealed providers. It is an inherent trade-off of cloud KMS, not a design flaw. The runbook (US-57.1's `NOTES.txt` + `pkg/secrets/README.md` threat-model update) documents this explicitly and recommends multi-region KMS key replicas (AWS KMS multi-region keys, GCP KMS keys in multiple locations) for deployments requiring high availability. The composite's static fallback does NOT mitigate this — the fallback only runs on prefix mismatch (legacy rows), not on KMS-primary decrypt failure.
...
- `charts/llmsafespaces/templates/NOTES.txt` (modified) — preflight warning when `kms.aws.enabled` but no credentials Secret referenced. Explicit note: "Under KMS, the master secret file mount protects only the Redis DEK cache; KMS-protected tables (provider_credentials, api_keys, org_sso_configs.client_secret) are encrypted by the configured KMS keys." Plus D9 availability-dependency documentation: sustained KMS unavailability degrades all KEK-dependent decrypts; multi-region KMS keys recommended for HA.
- `pkg/secrets/README.md` (modified) — add `AWSKMSProvider` and `CompositeProvider` rows; threat-model matrix updated to reflect exfiltration limitation under KMS; D9 availability-dependency section added.
...
11. `NOTES.txt` and `pkg/secrets/README.md` document the KMS-availability dependency (D9): sustained KMS unavailability causes all KEK-dependent decrypts to fail simultaneously; multi-region KMS keys recommended for HA deployments. The static fallback does NOT mitigate KMS-primary unavailability — it only runs on prefix mismatch.
