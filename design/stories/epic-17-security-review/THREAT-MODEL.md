...
|----------|-------|-------|------|----------|
| Security gaps (G1–G50) | 50 | 29 | 10 | 11 |

**Open gaps (require remediation):** G6, G9, G13, G21, G42, G43, G44, G46, G47
...
| Version | Change |
|---------|--------|
| 3.0 | Audit-driven reclassification of 6 gaps after a fresh code audit found stale rows. **Fixed (stale — validator/wiring already existed):** G29 (validateMountPath at secret_service.go:582, called from line 563), G45 (entrypoint-opencode.sh sources /sandbox-runtime/secrets-env not the legacy /sandbox-cfg/env), G50 (AuditedProvider wired at app.go:408,409,624). **Accepted (operator-side infrastructure dependencies):** G4 (mTLS needs service mesh or per-workspace cert), G30 (DNS exfil needs Cilium FQDN or Calico GlobalNetworkPolicy), G40 (agentd user-port auth — NetworkPolicy is the documented trust boundary). Updated STRIDE Proxy, Sandbox Pod, Database, and Workspace Network rows. Updated the highest-severity-open callout (G33/G34/G35/G50 closure history + new highest: G13, G21). Counts: 26 Fixed / 16 Open / 8 Accepted → **29 Fixed / 10 Open / 11 Accepted**. |
