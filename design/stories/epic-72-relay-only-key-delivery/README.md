...
| US-72.1 | `pkg/secrets.StagingProvider` — KMS-envelope prod / HPKE dev | done (#1407 merged) | M | — |
...
| US-72.6 | Migration: PVC scrub of legacy `auth.json` keys + e2e rogue-agent sweep (#820 close-out) | landed across two vehicles — #1537 (owner: agentd `scrub-legacy-keys` + boot-scheduled one-shot + `LegacyKeysScrubbed` condition/event mirror + `local/us-72-rogue-agent-sweep.sh`) and this PR (the #1536-pattern nightly sweep wiring + structural pins — the exit-criterion evidence lane); the sweep's first EXECUTED evidence begins with the first post-merge nightly run — #820 closes on that green run (Refs until then) | S/M | US-72.5 |
