...
| 62 | **SDK Refresh, API-Surface Parity & Publish** — close the drift between the four hand-written SDKs (TS/Python/Go/Java) and the current API server: refresh the stale `openapi.yaml`, bring Python/TypeScript/Java to typed-surface parity with Go (the de-facto reference), surface the session-queue/usage/agent-reload/question-permission routes no SDK exposes, publish to PyPI/npm/Maven with importable version metadata, and add the `/message` blocking+JSON contract test that is currently an unvalidated assumption. Closes the four open gaps recorded against Epic 14. Definition only — see `epic-62-sdks-refresh-publish/README.md` | Epic 14 (initial SDKs), Epic 41 (queue), Epic 12 (usage), Epic 16 (HITL) |
| 63 | **Inboard Session Queue — Adopt opencode V2 Session API** — eliminate the external Redis-backed message queue and its incident-scarred workarounds (message-id hack, 409-requeue, destructive abort, stranded-queue sweep) by having the proxy POST `delivery: "queue"` prompts directly to opencode's already-served V2 `/api/session/:sid/prompt` endpoint. opencode handles admission, promotion, and draining atomically in its own durable event log; abort becomes non-destructive (`/api/session/:sid/interrupt`). Loss of revoke is accepted. Definition only — see `epic-63-inboard-session-queue/README.md` | Epic 41 (current queue, superseded), Epic 29 (AgentClient), Epic 62 (US-62.6 SDK queue coordination) |
| 64 | **Triggers & Workflows** — deterministic DAG-structured pipelines (`script` / `agent` / `http` / `condition` nodes) that run inside workspace pods, fired by `cron` / `webhook` (HMAC-SHA256) triggers (manual runs go through run-create directly; no `manual` source type). **Authoring persona: human via UI/API (v1); in-workspace agent authoring deferred to v2.** **Triggering persona: programmatic/deterministic (cron, webhooks from external systems like "new job posting detected") — LLMs run inside workflows as `agent` nodes, not as trigger sources.** Orchestrator is a leader-elected controller reconciler (stateless API writes rows only); run-to-completion-or-fail (no resume); single-in-flight-per-workflow enforced by partial unique index (no TOCTOU, no configurability in v1); circuit breaker auto-disables triggers after N consecutive failures; separate `workspaceActivationTimeoutSec` (120s) prevents semaphore exhaustion on dead workspaces; cancel is immediate via context cancellation (not next-node-boundary); webhook fire-row + run-create are atomic (single tx, no orphan fire rows on reject). Script nodes use a function-call wrapper contract (`def handler(input) -> dict`), not stdin/stdout process model. Prompt templates use Go `text/template` (interpolation only — safe against attacker webhook payloads); condition expressions use expr-lang (author-trusted logic) — conditions route but pass their input through to successors unchanged. Cribbed primitives from Temporal/Argo/Inngest/Hatchet (retry policy, timeouts, 6-state enum, `error_code`, circuit breaker); deliberately NOT cribbed (child workflows, signals, dynamic DAGs, sagas, tags, queueing, N>1 concurrency). External agents author workflows/triggers via platform MCP tools (power-user path, not primary). v1 cut: 4 node types, 2 sources, 2 actions, user+org tiers. Definition only — see `epic-64-triggers-workflows/README.md` | Epic 30 (injection pipeline), Epic 29 (AgentClient), Epic 23 (controller single-writer); soft-dep Epic 51 (quota patterns) |
| 65 | **Agent Session Contract — Decouple the Platform from opencode** — a platform-owned session/message/event contract (`pkg/session/`) behind a single adapter seam (`pkg/agent.Adapter`), eliminating the opencode-specific leakage across 70+ platform files. Web, mobile, SDK, and MCP all consume the same contract; per-agent translation lives entirely in `pkg/agent/<name>/`. The 5-type part union (`Text`/`Reasoning`/`Tool`/`FileChange`/`Custom`) is validated against opencode, pi, aider, claude-code, strands, eino. The `AgentConfigWriter` seam (US-65.1) moves opencode's config-merge quirks (no hot reload, `OPENCODE_CONFIG` always-wins, `disabled_providers` relay injection, 20s stale window) behind `Apply(input) → restartRequired`. Steering (mid-turn redirect without abort) is a first-class `Admission` mode on `SendOpts`. Agent-specific operations (rewind, fork, stash) are capability-gated pass-through, not contract types. Deliberately NOT included: `Run` abstraction (message status already covers it), `AutoResolvePolicy` (config concern), `FooterState` event (client-computed), dedicated part types for todos/subagents/plan (all are `ToolPart`), typed rewind/fork results (`json.RawMessage` until a second adapter validates). Mobile + desktop + API are first-class peers; terminal pass-through stays as optional desktop power-user mode. 5 weeks sequenced: US-65.1 AgentConfigWriter → US-65.2 contract types → US-65.3 opencode adapter → US-65.4 proxy migration → US-65.5 hack deletion → US-65.6 repolint → US-65.7 MCP surface → US-65.8 frontend. No second adapter until funded (Rule 12). In implementation — US-65.1–65.7 merged (PRs #691–#727, 2026-08-09→2026-08-11 UTC); US-65.4/65.5 completed and superseded by #828 (the final dual-path retirement: adapter ctor-required, raw-proxy transport + dialect deleted, PRs #1347/#1354/#1357/#1361 + the final batch, 2026-09-12→13); US-65.8 frontend in progress — see `epic-65-agent-session-contract/README.md` and `design/0049_2026-08-09_agent-session-contract.md` | Epic 29 (AgentClient), Epic 30 (credential model). Resolves the C1/C2 coupling documented in Rule 12 and the Relay Config Subsystem fragility list |
