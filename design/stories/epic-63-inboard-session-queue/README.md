...
| F2 | The V2 session endpoints are defined in `packages/protocol/src/groups/session.ts:205-355`: `POST /api/session/:sid/prompt` (accepts `delivery`), `POST /api/session/:sid/interrupt`, `GET /api/session/:sid/event` (SSE) | Verified at v1.18.10: `HttpApiEndpoint.post("session.prompt", "/api/session/:sessionID/prompt", { payload: { prompt, delivery: SessionInput.Delivery.pipe(Schema.optional) } })` (line 205-210); `session.interrupt` at line 345. **Empirically verified (2026-08-09, spike):** `POST /api/session/:sid/prompt` with `{prompt:{text}}` + `delivery` → 200 `{data:{admittedSeq,id,sessionID,prompt,delivery,timeCreated}}`; `POST /api/session/:sid/interrupt` → 204 No Content (both in-flight and idle); `/event` serves SSE. All three confirmed against opencode 1.18.10 on a live workspace pod. Report: `worklogs/spike-us-63.1-report.md`. |
...
| F4 | **The TUI already uses the V2 prompt route today.** `sdk.client.session.prompt` (called by the TUI submit at `packages/tui/src/component/prompt/index.tsx:1093`) is generated to POST `/api/session/{sessionID}/prompt` with the `delivery` field. | `packages/sdk/js/src/v2/gen/sdk.gen.ts:5622-5647` — `public prompt(...)` posts to `/api/session/{sessionID}/prompt` with `delivery?: "steer" \| "queue"`. **Empirically verified (2026-08-09, spike):** the V2 prompt route is exercised end-to-end — admission 200, promotion (`session.next.prompted` on SSE), and a full turn lifecycle (`step.started` → `text.delta` → `step.ended`) all observed. **CAVEAT — see F18:** the request body shape on 1.18.10 is `{prompt:{text:"..."}}`, NOT the parts-based shape; the proxy V2 client (US-63.2) must use the text shape. |
...
| F8 | opencode's V2 `interrupt` is non-destructive: kills the running fiber, fails open tools/assistant, but **preserves** admitted-but-unpromoted input rows for `resume` | `packages/core/src/session/run-coordinator.ts:94-101` (`entry.pendingWake = false; Fiber.interrupt`); test `packages/core/test/session-runner.test.ts:1901` (`"preserves durable queued input for a later wake after interruption"`). **Empirically verified (2026-08-09, spike):** `POST /interrupt` returns **204 No Content** both during an in-flight turn and on an idle session. The session continued processing queued input after the in-flight interrupt (a subsequently-promoted essay turn ran to completion per SSE), consistent with non-destructive semantics. (Caveat: the spike did not assert the specific queued row's survival via a post-interrupt history probe; the 204 + continued queue processing is indirect confirmation. A direct probe is a cheap US-63.4 test add.) |
...
| F13 | **V2 events DO reach the proxy's existing SSE subscription.** The V1 `/event` route is bridged from V2 via `EventV2Bridge.Service` (`packages/opencode/src/server/routes/instance/httpapi/handlers/event.ts:91`) → `events.listen` → `GlobalBus` → SSE. V2 `PromptAdmitted`/`Prompted` appear on `/event` with their base type strings. | `handlers/event.ts:32-40`; `event-v2-bridge.ts` emits to `GlobalBus`. **Empirically verified (2026-08-09, spike):** CONFIRMED. The proxy's existing `/event` SSE stream carries V2 events — 10 events observed in a 12s window including `session.next.prompt.admitted`, `session.next.prompted`, and the full turn lifecycle (`step.started`/`text.started`/`text.delta`/`text.ended`/`step.ended`). **Wire format note for US-63.5:** events are `data: {"id":...,"type":"<type>","properties":{...}}` lines (type inside the JSON payload), NOT a separate `event:` line — the bridge must parse the data-line JSON. |
| F14 | **V2 event wire type strings** (load-bearing for US-63.5): `session.next.prompt.admitted` (admitted, `promoted_seq` null) and `session.next.prompted` (promoted). Durable events also carry a versioned type on the `sync` channel, but the primary `/event` stream uses the base string. | `packages/schema/src/session-event.ts:86-99`. **Empirically verified (2026-08-09, spike):** CONFIRMED verbatim. Observed `session.next.prompt.admitted` (properties: messageID/sessionID/prompt.text/delivery/timestamp) and `session.next.prompted` (same property set) on `/event`. The admission event carries the full prompt payload, which is useful for US-63.10 (fresh-load queue visibility via accumulated admission events). |
| F15 | **`resume` is NOT exposed in the V2 HTTP API.** Endpoint enumeration yields: `list, create, active, get, switchAgent, switchModel, prompt, compact, wait, revert.{stage,clear,commit}, context, history, events, interrupt, message`. No `resume`. `session.wait` is a stub returning `OperationUnavailableError`. | `packages/protocol/src/groups/session.ts`; `packages/core/src/session.ts:421-424` (`wait` returns OperationUnavailableError). **Not directly probed by the spike** (the harness does not enumerate endpoints for a `resume` route). The code-reading finding stands unchallenged; F16's empirical confirmation below makes F15 load-bearing for US-63.9 regardless of a direct probe. |
| F16 | **Nothing auto-drains queued input on pod restart.** `runner.run` is triggered only by `execution.wake` (on a new prompt) or `execution.resume`. opencode's startup has no scan for sessions with pending `SessionInput` rows. Durable rows survive in SQLite but strand until the next prompt arrives. | `packages/core/src/session/runner/llm.ts:383-406` (the only `run` entry); no startup hook found via grep. **Empirically verified (2026-08-09, spike):** CONFIRMED. After SIGKILL of the opencode process (agentd restarted it in-place, same pod, `/global/health` back to 200 in ~6s), a 60s SSE watch on `/event` showed ONLY `server.heartbeat` events — zero `session.next.*` activity — and the marker prompt's assistant reply was absent from session history. Queued input did NOT drain autonomously after restart. **This is the load-bearing input for US-63.9: a drain trigger MUST ship before US-63.7 deletes the stranded-queue sweep.** |
| F17 | **V2 `prompt` can return 409 `PromptConflictError`** if a caller-supplied `id` collides with an existing durable row; it can also 404 if the session doesn't exist (V1 `prompt_async` auto-creates, V2 does not). The proxy must omit `id` and operate only on existing sessions. | `packages/core/src/session.ts:380-381` (conflict); `:363` (`result.get` dies if not found). **Empirically verified (2026-08-09, spike):** the spike omits caller-supplied `id` (per F17's own guidance) and observed only 200 admissions — no 409, no 404 — across 8 prompts on an existing session. The 409-on-collision and 404-on-missing-session paths were not directly triggered (would require sending an `id` or a bogus sid); the happy path confirms the "omit id" recommendation works. Direct probes are a cheap US-63.2 test add. |
| F18 | **NEW (spike 2026-08-09): opencode 1.18.10's V2 prompt body shape is `{prompt:{text:"..."}}`, NOT the parts-based `{prompt:{parts:[{type:"text",text:"..."}]}}`** that the original US-63.1 spec and this harness assumed. The parts shape is the Epic 65 part contract (design 0049), newer than the bundled binary; 1.18.10 rejects it with HTTP 400 `InvalidRequestError: Missing key at ["prompt"]["text"]`. **This is a load-bearing input for US-63.2.** | Spike 2026-08-09: POST `/api/session/:sid/prompt` with parts body → 400; same endpoint with `{prompt:{text}}` → 200 `{data:{admittedSeq,id,sessionID,prompt:{text},delivery,timeCreated}}`. US-63.2's V2 client MUST POST `{prompt:{text}}`; a future opencode bump to the parts contract (Epic 65) will require client adaptation — pin the contract and mirror the `agent_config_writer_schema_test.go` pattern. Report: `worklogs/spike-us-63.1-report.md` (Phase 2b). |

> **Stress-test note (2026-08-02; updated 2026-08-09 with empirical results):** F15-F16 were the epic's largest residual risk and motivated US-63.9 (stranded-input recovery) and US-63.10 (fresh-load queue visibility). **F16 is now empirically CONFIRMED (not just code-read): queued input does NOT auto-drain after an in-place opencode restart — US-63.9's drain trigger is mandatory and cannot be descoped.** F13-F14 de-risk US-63.5 and are now both empirically confirmed (exact wire strings observed). F17 corrects US-63.2's `id` handling and its happy path is confirmed. **F18 (new) is a fresh risk surfaced by the spike: the V2 prompt body shape differs from the parts-based contract the original design assumed; US-63.2 must target the 1.18.10 `{prompt:{text}}` shape and pin it.**
