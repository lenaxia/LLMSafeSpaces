...
4. **Provenance**: the running controller image's commit label equals the tag's commit (the #1548 drift check, automated — digest↔commit binding via the image label the release pipeline already stamps).
...
- **Defect 3**: gate-or-grant — **grant** (name-scoped `update` on `llmsafespaces-free-models` + unscoped `create`, mirroring the pins shape) rather than gate the binary path: the refresher's catalog is posture the platform always wants; the values flag then governs only the loop cadence, not the RBAC shape. (Decision recorded here for review; the alternative — gate the code path on the flag — is the smaller diff but re-couples boot behavior to values drift, which is this incident's middle name.)
