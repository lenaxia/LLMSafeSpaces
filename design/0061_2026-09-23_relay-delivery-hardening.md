...
**#1546 — the posture defects.** Under the chart's own mandated posture (`rbac.scope=namespace` forced by the relay-only guard), the release was DOA: the router's keypair bootstrap cannot `create` through a resourceNames-scoped rule (the apiserver ignores `resourceNames` for create — the chart documents this for agentd pins and then contradicts it here); the pins informer needs `list/watch` the pins Role never granted; and `inferenceRelay` (a cluster-scoped CRD watch) cannot coexist with the namespace posture at all. The fourth report — the free-models refresher "running unconditionally" — was the drifted binary, not a shipped desync: the refresher is flag-gated in every merged tree and the chart renders flag+RBAC from the same values key (§5's re-derivation; the #1548 RCA's elimination argument). Coverage history, precisely: Defect 1 (the keypair rule) renders only under relay-only — new at the flip, so no prior install exercised it at all; Defects 2/3's configmap access and the design gap were masked by the cluster-scope grant's broader reads. Either way, the least-privilege posture the flip mandates had **never actually been exercised** with delivery overlays enabled.
...
- **Strict mode's detection downgrade, stated honestly:** the fallback counter is structurally zero in strict mode (the fallback is disabled), so a post-strict staging death produces fail-closed zero-entry batches with only M4's condition — no metric, no fleet-level alert. Two honest options, and the design picks the first: **(a)** a degrade-class counter series `relay_degraded_batches_total{workspace,reason}` emitted on every fail-closed batch (riding work that already happens — the degrade reason is already computed; the series survives both modes and makes AC2 mode-independent); (b) strict-mode detection rides M4's condition alertability alone. **(a) ships with M2** — one more counter on the same emit path, not a fifth mechanism. the fallback restores the pre-flip delivery — raw keys in pod files. That is the S1/S2/S3 surface design 0058 spent an epic removing, so the fallback trades confidentiality back for **availability** deliberately and *observably*: every instance is counted, alerted, and audited. The migration-mode default is the honest posture for a fleet whose staging producer just proved it can be silently absent; strict mode is earned, not assumed.
...
3. **Zero forbidden**: no `forbidden` line in any pod log in any namespace (kills Defect 2 and the design-gap flood, and covers the refresher-RBAC-starvation CLASS generically — silent RBAC starvation becomes a red gate; Defect 3 itself shipped no defect, §5's re-derivation).
...
> **Deviation note (r2 of the RBAC-fix PR, #1552, recorded per the §5 precedent):** the shipped grant is WIDER than this section's letter, with a functional basis — the reconciler's complete CRD lifecycle needs `update` on the main resource (the finalizer add/remove: `reconciler.go:129/832` — controller-runtime `Update` PUTs the MAIN resource, so no `/finalizers` subresource grant is needed or granted) and `update` on `/status` (`Status().Update`: `:363/:711/:749`). Read-only left the fleet inert-under-green (the informer flood gone, a real CR's first reconcile forbidden). Secrets remain NEVER (§4.3): the fleet's VM-key Secret writes (`:647/:651`) ride the NAMESPACED inferenceRelay grant as before. **And the honest residual, made fail-loud at install time:** the reconciler's `Owns(&corev1.Secret{})` registers a cluster-wide Secret informer; with `controller.watchNamespaces` unset (the default cluster-wide cache) that reflector LISTs secrets cluster-wide — granted nowhere under the namespace posture, a controller crash-loop (the chart's own cluster-block comment documents the failure). The chart therefore REFUSES `inferenceRelay.enabled=true` under namespace scope without `controller.watchNamespaces` (the relay-only guard's pattern) — fleet-under-mandated-posture coexistence requires the scoped cache, stated here rather than discovered in a crash-loop. Tracked wedge: #1551 family.
...
- [ ] M2: the builder table (absent/expired handoff → raw-key + counter + audit; present → token); strict mode preserves the fail-closed zero-entry behavior; BOTH counters (`relay_fallback_deliveries_total` migration-mode, `relay_degraded_batches_total` both-modes) render and fire; the migration-mode default lands with the flip criteria in the runbook. (#1548 AC2 is mode-independent via the degrade-class series — the map below holds in strict mode too.)
