...
| `TestSanitizeSchemaViolations`: build failing instances whose *values* would appear in raw jsonschema messages (pattern-mismatched string, format-invalid value, extra property); assert the persisted `action_result` contains **no instance-derived substrings** (typed `{jsonPointer, keyword, message}` only) and respects the hard 4 KiB cap — truncation marker present; retained records keep their pointers; a record that cannot fit is dropped and counted in the marker; the deep-pointer near-cap instance exercises the drop-record path down to the bare `{"code","inputFrom"}` skeleton floor | §3.5 payload contract — the #1419 no-instance-values rule and cap invariant | unit (`pkg/workflows`) |
...
2. **Should the user/org API trigger-create also enforce workflow *existence*** (the pod path already does, `pod_automation.go:356`)? V3 fetches only when an opt-in is present, so un-opted ghost wiring remains creatable via the API precisely to keep #1412's fire-time loudness (and its R4 e2e row) intact. Confirm that asymmetry is intended, or accept the R4 fixture rework.
   **AMENDED — RESOLVED (both halves):** the create half was resolved by the 35597973572 contract ruling (create answers a ghost `workflowId` with the named 400 "target workflow not found"; the R4 e2e row was reworked to create-valid-then-delete, riding #1440's FK-SET-NULL). The update half was resolved by #1519: the update path mirrors create's contract with an owner-scoped existence check on the post-patch MERGED view inside the `touchesMapping` block — retargets to nonexistent or cross-owner workflows, and mapping-touching patches that surface stored ghosts, answer the same named 400; non-mapping patches on legacy rows stay editable, and fire-time loudness remains only for never-re-touched legacy wiring.
