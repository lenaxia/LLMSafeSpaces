...
| `firstboot_two_replica_adopt` (simultaneous cold start → one adopted keypair, fingerprint match), `rotation_dual_key_window_bounded` (both replicas resolve old+new keys across re-seal; window ≤ watch bound), `prior_key_retention_expiry` (uniform drop after the interval), `dr_keypair_loss_failclosed_recovery` (Secret loss → `CredentialStale` window → re-seal recovery), `envelope_keyid_aad_bound` (tampered `keyID` fails GCM auth) | §4.2 replica-symmetric bootstrap/rotation/DR; metadata-only confirmation | integration, fault-injection |
