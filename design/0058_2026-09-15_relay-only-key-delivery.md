...
- **Dev (HPKE):** no cloud dependency — the router keypair is generated at first boot; the controller seals with the router's public key (HPKE seal), so the controller *never* holds decrypt capability in dev. Wire format is a versioned envelope (`stg:v1:` prefix, algorithm discriminated) so prod/dev are interchangeable per deployment. Pinned specifics (the 2026-09-15 review's finding 2): **library** — Go has no stdlib HPKE and `go.mod` carries no HPKE dependency today (verified: no `circl`/`hpke` in `go.mod`); the candidate is `github.com/cloudflare/circl` (HPKE, RFC 9180), subject to US-72.1's dependency review (govulncheck + Trivy already gate CI; the review must record version pin + license). **Key distribution — RBAC-authenticated, not trust-on-first-use:** the router generates the keypair at first boot and writes the private key to the `llm-relay-hpke-key` Secret — under the §4.3 name-scoped write carve-out (create/update on exactly the two keypair Secret names; RBAC is the authenticity bound); the public key is published to `llm-relay-hpke-pub`, which the controller reads. **Rotation (controller-driven, no availability window):** the reconciler requests rotation on the router's internal API; the router generates the new keypair, updates the **same two Secret names in place** (carve-out intact), publishes the new public key, and retains the prior private key **in memory only**, resolving envelopes under either key (envelopes carry a plaintext `keyID` discriminator) until retirement. The controller re-seals every staged envelope against the new public key, confirms completion **by metadata alone** — every envelope's `keyID` equals the new key id; no decrypt, no resolve probe, no capability change — then requests retirement and the router drops the in-memory prior key. **Disaster recovery** (keypair Secret lost or corrupted): the router regenerates at next boot into the same names; envelopes sealed to the lost key are unresolvable until reconcile re-seals them — an honest, bounded, fail-closed window surfaced as `CredentialStale`, distinct from routine rotation which never has one.
