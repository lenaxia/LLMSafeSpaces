...
└─────┬──────┘                          └───────▲────────┘  │ │ baseURL = router│ │
      ...
      ▼                                         │ cache     │   token only (D3)    │
┌─────────────────────────┐  resolve per req:   │          │                      ▼
│ llm-relay-router ×2     │◄────────────────────┘          │   POST /v1/…, Bearer TOKEN
│ (llm-relay ns)          │◄──────────────────────────────────────────────────────────┘
...
- **Minted by the router** (`POST /internal/v1/tokens`, authenticated by a controller-held Secret — the API/controller are the only mint callers). The router is the validator; making it the minter keeps exactly one holder of the signing key.
...
- **Scope enforcement per request (all three, all mandatory):** workspace (token↔staged-Secret binding), baseURL (the token names exactly one upstream — SSRF-proof by construction, same pinning property as the withdrawn broker draft), model allowlist (router parses the request's `model` field against the allowlist; the allowlist already exists per-binding at `pkg/secrets/credential_store.go:50` and is already enforced batch-side at `injection.go:531+` — the router re-enforces it, defense-in-depth).
- **Validity = HMAC ∧ staged-Secret-present ∧ not-expired.** The staged-Secret conjunct is what makes revocation = Secret deletion (D2): an HMAC-valid token without ciphertext is a 401 with reason `credential_stale`.
...
| `CredentialStale` | token expired / staged Secret absent / batch not applying the staged revision (resolve fails closed) | controller, from reconcile + agentd degrade codes |
...
| A5 | A per-binding model allowlist already exists and is enforced batch-side | `pkg/secrets/credential_store.go:50`; `pkg/secrets/injection.go:531+` (`applyModelAllowlist`) |
...
3. **Self-hosted single-node deploys without cloud KMS:** HPKE dev mode doubles as the no-KMS production mode (documented trade: no cloud-side key protection). Confirm acceptable.
