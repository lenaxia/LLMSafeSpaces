...
// apiKeyProv construction (app.go:631-655) for the self-hosted providers:
//  1. --sealed-key-file + --passphrase-file (sealed) — used directly, mirroring
//     the API (the sealed path is NOT multi-version-upgraded).
//  2. static, from the same env vars the API reads — built as a multi-version
//     provider {1: dek-cache, 2: master-kek} with version 2 active, EXACTLY as
//     the API does. This matters: the API's static apiKeyProv stamps version 2
//     and encrypts with the "master-kek" purpose key. Building a single-key
//     provider here would stamp version 1 (the "dek-cache" slot in the API's
//     provider) and produce inconsistent key_version metadata.
func buildRootKeyProvider(sealedKeyFile, passphraseFile string) (secrets.RootKeyProvider, error) {
	...
	// Derive both purpose keys (mirror deriveServerKey) and build the multi-
	// version provider the API uses: version 1 = dek-cache (legacy decrypt),
	// version 2 = master-kek (active encrypt).
	dekCacheKey, err := secrets.DeriveKEKFromKey(master, []byte("llmsafespaces-server"), "dek-cache")
	if err != nil {
		zero(master)
		return nil, fmt.Errorf("derive dek-cache purpose key: %w", err)
	}
	masterKEK, err := secrets.DeriveKEKFromKey(master, []byte("llmsafespaces-server"), masterKEKPurpose)
	zero(master)
	if err != nil {
		return nil, fmt.Errorf("derive master-kek purpose key: %w", err)
	}
	p, err := secrets.NewStaticKeyProviderMultiVersion(2, map[int][]byte{
		1: dekCacheKey,
		2: masterKEK,
	})
	zero(dekCacheKey)
	zero(masterKEK)
	if err != nil {
		return nil, fmt.Errorf("static multi-version provider: %w", err)
	}
	return p, nil
}
