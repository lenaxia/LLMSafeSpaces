...
/**
 * Live-shaped opencode message.part.updated event for a tool part —
 * envelope matches the raw SSE capture (2026-08-20, opencode 1.18.10):
 * {type:"agent.event", event_type, data:{id, type, properties:{sessionID, part}}}
 * part carries id (prt_...), messageID, callID, state{status, input,
 * metadata.output, time.start}.
 */
function makeToolPartUpdatedEvent(over: {
  ...
  return {
    ...
    data: {
      id: `evt_${over.partId}_${over.timeStart}`,
      ...
      properties: {
        ...
        part: {
          ...
          state: {
            ...
            metadata: { output: over.output },
            time: { start: over.timeStart },
          },
        },
      },
    },
