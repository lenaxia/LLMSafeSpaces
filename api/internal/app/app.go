...
func New(cfg *config.Config, log *logger.Logger) (*App, error) {
	...
	// #1314: the adapter split — V1 and V2 are separate types, not a
	// flag on a shared adapter. Delivery is V1 (the admitter POSTs
	// /session/:sid/message); the AdapterV1 reads history from the V1
	...
	var dekCacheClient *redis.Client         // closed on Shutdown
	{
		...
		wfScheduler = &apiwf.Scheduler{
			...
			// #1452: routine fires write the session_index row for
			// preserved sessions — without it the sidebar list, which
			// serves the index only, never shows them.
