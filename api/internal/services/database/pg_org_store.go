...
func scanSSOConfig(row *sql.Row, cfg *types.OrgSSOConfig) error {
	...
	if err := row.Scan(
		...
		&cfg.ClientID,
		&cfg.ClientSecret,
		stringArrayScan(&cfg.ClaimedDomains),
		stringArrayScan(&verifiedDomains),
		&verificationToken,
		&cfg.AutoProvision,
		&groupMappingBytes,
		&cfg.CreatedAt,
...
func (s *PgOrgStore) UpsertSSOConfig(ctx context.Context, config *types.OrgSSOConfig) error {
	...
	_, err := s.db.ExecContext(ctx,
		...
		pgStringArray(ssoDomainsParam(config.ClaimedDomains)), pgStringArray(verified), token,
...
func (s *PgOrgStore) ListSSODomains(ctx context.Context) ([]types.SSODomain, error) {
	...
	for rows.Next() {
		...
		var domains []string
		if err := rows.Scan(&slug, &name, stringArrayScan(&domains)); err != nil {
