...
func newRouterFixtureWithProxy(t *testing.T) (*gin.Engine, *mockServices, *handlers.ProxyHandler) {
	...
	proxyHandler, err := handlers.NewProxyHandler(k8sMock, log, "default", nil, nullTestAdapter{})
	require.NoError(t, err)
...
func TestListWorkspaceSessions_StatuszGroundTruth_StaleActiveShowsIdle(t *testing.T) {
	...
	proxyHandler, err := handlers.NewProxyHandler(k8sMock, log, "default", httpClient, nullTestAdapter{})
	require.NoError(t, err)
