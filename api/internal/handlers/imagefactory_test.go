...
type fakeIFStore struct {
	...
	orgIDByUser  map[string]string // GetUserOrgID lookup
	err          error
...
func (f *fakeIFStore) GetConfig(ctx context.Context, id string) (imagefactory.Config, error) {
	...
	return imagefactory.Config{}, errors.New("not found")
}
...
func (f *fakeIFStore) GetUserOrgID(ctx context.Context, userID string) (string, error) {
	return f.orgIDByUser[userID], nil
}

func newIFRouter(t *testing.T, store imageFactoryStore) *gin.Engine {
	t.Helper()
	gin.SetMode(gin.TestMode)
	r := gin.New()
	// Stub the auth middleware: tests set userID via the X-Test-UserID header,
	// and this middleware copies it into the gin context as the real
	// AuthMiddleware would. Empty header = unauthenticated (handlers see "").
	r.Use(func(c *gin.Context) {
		c.Set("userID", c.GetHeader("X-Test-UserID"))
		c.Next()
	})
	h := NewImageFactoryHandler(store)
...
func TestIF_ListConfigs_Happy(t *testing.T) {
	...
	store := &fakeIFStore{
		...
		orgIDByUser: map[string]string{"user-1": "org-9"},
	}
	r := newIFRouter(t, store)
...
func TestIF_ListConfigs_EmptyReturnsArrayNotNull(t *testing.T) {
	t.Parallel()
	store := &fakeIFStore{configs: nil, orgIDByUser: map[string]string{}}
	r := newIFRouter(t, store)
	w := doAuthed(t, r, "GET", "/api/v1/image-factory/configs", "user-1")
	require.Equal(t, http.StatusOK, w.Code)
