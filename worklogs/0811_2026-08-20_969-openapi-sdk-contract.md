...
### Round 2: the Go no-op — fourth str.replace incident, this time caught by review + CI

The round-1 commit claimed all four SDKs updated; the Go edits had silently no-opped (the python patch's old-string had different field alignment than the file — replace matched nothing). The reviewer's SDK-by-SDK check and the SDK Contract Tests both caught it. Root cause is the recurring one from #947/#949: **python str.replace + no post-verification**. The Go types are now landed via direct edit, grep-verified (4 hits: InputRequest/InputOption/ToolRef/OldPath), build+test green before commit. Process note recorded: patches to Go code go through the edit tool or include a grep verification step in the same command.
