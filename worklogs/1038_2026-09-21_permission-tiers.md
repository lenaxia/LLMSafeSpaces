...
- **The design/stories/epic-16 sweep miss (r8 finding 3): corrected with dated notes** — r9 adds the FOURTH site (:258 — the r8 disposition said "all three" while the finding named four; scope-narrowing in a fix claim, the same class). Historical verification records preserved, the live top-level shape named at all four.
- **r9 — the pin example corrected EMPIRICALLY**: the r7 review's own "/sym/*" under-drop example was false (transcribed by me without verification — ^/sy.m/.*$ demands a char between "sy" and "m"); the true verified under-drop is "/symm/*" (tierMatch true, filter keeps it). Fixed in the pin comment and here; the verification ran through the shipped matcher in this tree before this entry was written.
