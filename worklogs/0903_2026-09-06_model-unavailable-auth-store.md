...
2. **The boot-time relay writer (pre_boot_relay.go → updateAuthJSONForRelay — the shared helper the in-pod injector also uses) created auth.json 0640 uid-2000** — the #1119 fix made it readable, but opencode (uid 1000) must also WRITE it (its auth subsystem persists at boot and PUT /auth); PermissionDenied → 500 → provider init fails → the registry comes up EMPTY → "Model unavailable" for EVERY provider (relay included). Live-proven: PUT /auth 500 with "PermissionDenied: Failed to write auth data"; with the file made uid-1000-writable, the same PUT returned 200. (In sidecar mode the in-pod injector goroutine never runs; the boot writer is the pre-boot relay.)
...
4. Intermittency explained: pods where opencode won the write race (created the file itself, uid-1000-owned) worked until recreation; relay-won pods wedged until recreation — the store is pod-scoped tmpfs behind the PVC-side symlink (nothing persists on the PVC but the dangling link), and the wedge re-armed each boot (the pre-boot relay rewrote 0640). The 863 pod: wedged since 2026-08-29 (the 0600 class) — old pods kept their working store from the era before the regression. NOT a 0.27.x regression.
