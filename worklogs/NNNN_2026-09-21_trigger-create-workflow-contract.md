...
Run 35597973572's finding: valid create + ghost `workflowId` → opaque 500 in 4.5ms (store-layer FK shape). Fix per the ruling: a pre-flight existence check via the store's owner-scoped `GetWorkflow` — `400 {"error": "target workflow not found"}` (the update path's in-family wording), **ordered after cron validation** so invalid exprs keep answering the cron error (R1a's contract). Review r1 sharpened the shape: the V-matrix pre-fetch (validateTriggerInputMapping, an identical GetWorkflow earlier in create) already 500s on any non-NotFound fault and 400s opted-in ghosts — the new block's ONLY live surface is un-opted + NotFound, so it carries just the 400 arm (the first draft's 500 arm was provably dead and its test misattributed; both removed). The FK stays the integrity anchor (ON DELETE SET NULL, the #1440 loud path) — this is the contract check, not a replacement. **OWNER-OVERRIDE FLAG**: this overturns a mock-only contract — `TestTriggerInputMapping_V6`'s ghost arm pinned "un-opted ghost stays creatable", but that only ever held in the FK-less mock; the real DB answered those creates with the opaque 500, so the two contracts were already inconsistent in production. The V6 arm now pins the ruled 400 (its guard-scope intent is preserved by the neighboring schema-less-workflow arm).

Tests (TDD red→green): `TestTriggerCreate_NonexistentWorkflow_Named400` (400 + nothing stored — reviewer mutation-verified red pre-fix) and `TestTriggerCreate_CronValidationErrorOutranksWorkflowCheck` (error ordering). Plus the live-API unhappy row the review required: **R1c** in the automation suite POSTs a ghost workflowId and asserts the named 400 (ledger-pinned in RowsAndAssertions) — the pin that binds the ruling to the real DB shape. Existing-test fixtures completed (the mock never modeled create-path workflow existence): seeds in Cron/Webhook/V7/TargetPresenceGuard + the pod-automation pair's trigStore (unified-store topology noted in-comment).
