...
### Review iteration 8 (AI reviewer, CHANGES_REQUESTED → two blocking + two minors → fixed)

Both validated real, fixed in commit 9: (1) removing the cross-Secret assert (round 7) deleted the only detector for a dual-replica DR split (equal-generation interleaving priv_A/priv_B/pub_B/pub_A leaves every pinned check passing while nothing resolves) → DR recovery is now a **self-issued rotate** with a generation-preconditioned two-Secret update: only the replica whose private-key update wins writes the pub; the loser aborts before pub and re-adopts the winner — split unreachable by construction; `dr_dual_replica_recovery_single_keypair` test added. (2) the DR/residual window was "bounded" with no terminating trigger → reconcile-time predicate wired: re-get the pub, compare generation vs last-sealed; changed → re-seal; unchanged + persistent `CredentialStale` → anti-storm-bounded (≥10m) rotate escalation; `dr_window_reconcile_terminates` test added. Minors: generation monotonicity pinned non-strict (≥ — informer re-list redelivery); epic wording "cross-Secret generation checks" → controller-side generation checks (seal-time vs rotate response; reconcile-time vs last-sealed).

### Review iteration 9 (AI reviewer, CHANGES_REQUESTED → one blocking + one minor → fixed)

Blocking, validated real: `dr_window_reconcile_terminates` was homed in US-72.2 (router story) but the predicate is controller-side and its subject (staging reconcile) does not exist during US-72.2's window → moved to US-72.3's test plan + a predicate AC added there (re-get pub, generation vs last-sealed, re-seal on change, anti-storm-bounded escalation); US-72.2's plan now scopes itself to the router side with an explicit ownership note. Minor: the rotation path's generation precondition was only asserted by DR attribution → stated in the rotation bullet itself (current generation == the receiving replica's loaded generation; concurrent rotate/DR loses the private-key write and aborts before pub). Reviewer's second pass self-discarded nine candidates (escalation-spacing vs tunable retention, no total attempt bound, pub NotFound, last-sealed loss on restart, AC-prose granularity, idempotent re-seal, non-strict ≥, worklog integrity, test-name drift).

### Review iteration 10 (AI reviewer, CHANGES_REQUESTED → one blocking + one minor → fixed)

Blocking, validated real: the escalation trigger was unqualified over CredentialStale causes — revocation (D2's DESIGNED terminal state) and delivery faults make the condition persist forever, so one revoked workspace would drive a non-terminating cluster-wide rotate/re-seal storm (rotation succeeds, bumps generation cluster-wide, fires every other workspace's re-seal leg, repeats every ≥10m forever) → escalation qualified to resolve-failure-class staleness under an intact lineage (envelope Secrets present AND batch applying the staged revision); suppression for revocation-class/delivery-class pinned in §4.2, §7, and the US-72.3 AC/test. Minor: `pub_sealtime_generation_validated` is controller-side but sat in US-72.2 with no AC owner → moved to US-72.3's test plan + a seal-time-validation AC added there (pairs with the rotate leg).

### Review iteration 11 (AI reviewer, CHANGES_REQUESTED → one classifier gap + three minors → fixed)

Classifier gap, validated real: the round-10 partition was non-exhaustive over §4.6's cause taxonomy — token-expiry staleness passed the operational test (envelopes present, batch applied) but rotation cannot repair it (rotation re-seals envelopes, never mints tokens), and it fires in a DESIGNED state (the #852 deferral window: fresh token applied, restart waiting behind a busy session past TTL) → cluster-wide churn, session-bounded → escalation cause-set narrowed to exactly corruption/wrong-pub-class under an intact **spawn-layer** lineage (reads `spawned_rev`-class terminal verification, not the batch-apply anchor — the deferral window reads as pending-delivery and never escalates); token-expiry-class added to the suppressed set (renewal owns it). Minors: corruption class added to §4.6's + US-72.3's condition enumerations (the escalating class was absent from both); "correct steady state" rationale corrected (true for revocation only; delivery-class is a fault rotation cannot repair); US-72.2 ownership note names BOTH controller-side tests now in US-72.3.

### Review iteration 12 (AI reviewer, CHANGES_REQUESTED → one blocking + one minor → fixed)

Blocking, validated real: my round-11 edit updated US-72.3's TEST PLAN but silently missed the story's AC paragraph (stale replace target) — the AC still pinned the retired round-10 classifier (batch-apply anchor, no token-expiry suppression), contradicting the story's own test plan nine lines below → AC rewritten to match §4.2/§7/the test plan: corruption/wrong-pub-class only, spawn-layer (`spawned_rev`) anchor, three-class suppression (revocation / delivery incl. #852 deferral / token expiry). Minor (worklog honesty, append-only): iteration-11's ledger over-claimed — "added to §4.6's + US-72.3's condition enumerations" was true only for §4.6; the US-72.3 conditions-AC half landed in THIS commit, not round 11. Also noted: the round-12 review run for b2a06f49 initially failed at the "Verify review submitted" step (submission transient) and was `gh run rerun` — same commit re-reviewed.

### Review iteration 13 (AI reviewer, CHANGES_REQUESTED → one skipped fix + one ledger error → fixed)

Two findings, both mine to own: (1) the conditions-AC clause (add the corruption/wrong-pub class to US-72.3's `CredentialStale` cause enumeration) was prescribed in round 11, re-verified unaddressed in round 12, and skipped again in round 12's fix — root cause: my scripted replace never matched the file's actual wording ("batch not applying staged revision", no "the"), and I verified the TEST-PLAN and predicate-AC edits but not this one; without it, the predicate's sole escalating class is never surfaced by the condition, dead-on-arrival for the one cause it exists for. Fixed now, verified by re-read. (2) iteration-12's ledger claimed "the US-72.3 conditions-AC half landed in THIS commit" — false; what landed was the predicate-AC rewrite (a different bullet). This entry is the correction; the conditions clause lands in THIS commit (13), three rounds late.
