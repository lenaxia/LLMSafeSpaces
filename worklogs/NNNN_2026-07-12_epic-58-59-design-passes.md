...
- **CORRECTION (2026-07-12, post-PR-#550 review — finding C8):** The "false alarm" conclusion above was itself wrong. Re-reading `pkg/secrets/rotation.go:183` (`RotateAll` walks only `{"provider_credentials", "api_keys", "org_sso_configs"}`) and `cmd/rotate-kek/main.go:38` (`validTables` is the same set) confirms `user_keys` is **absent** from the walk. The premise the review relied on — "the walk over `user_keys` already exists" — is false; adding `user_keys` to the walk (plus a new `validTables` entry and a per-row `dek_source` branch) is new work for Epic 58, not an additive branch. This is a real gap that the mandatory Rule 11 review gate missed, and it raised the Epic 58 estimate (see Recovery + Files in the Epic 58 doc). The original bullet is retained verbatim above per the no-retroactive-rewrite rule in README-LLM.md.
- **CORRECTION (2026-07-12, post-PR-#550 review — finding C9):** The first bullet in this section claims the Epic 58 doc's "open-questions section asks the operator to decide whether to surface this to existing SSO users." At the time this worklog was written that was inaccurate — Epic 58's open-questions list had only 4 items and none mentioned user notification/disclosure. A 5th open question on user notification/disclosure has now been appended to the Epic 58 design doc to make that remediation claim real; see "Open questions for sign-off" §5 in `design/stories/epic-58-sso-server-kek-default/README.md`. Original bullet retained verbatim per the no-retroactive-rewrite rule.
