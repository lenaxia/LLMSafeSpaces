...
- `go test ./cmd/migrate-passkey-dek/ -v` — 13/13 pass:
  ...
  - `IsUUIDLike` — email vs UUID heuristic
  - `DecodeMasterRaw` — hex vs raw bytes
  - `LoadPassword_EnvFallback` / `LoadPassword_FileTrimsNewline` / `LoadPassword_None`
  - `BuildRootKeyProvider_StaticIsMultiVersion` — locks version-2 parity with the API
  - `BuildRootKeyProvider_NoMaterial` — errors when no master KEK configured
  - `MigrateUser_MultiVersionProvider_StampsVersion2` — end-to-end version-2 stamp
...
- `worklogs/0662_2026-08-02_dek-server-kek-migration-script.md` — this entry (added)
