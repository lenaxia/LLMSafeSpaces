...
1. unarmable→85-in-window: the wiring (source-truth pin). [r4 correction: the r0 text claimed the fail-loud matrix "now reads the named constant" — FALSE, no guard test was touched; the window's 30s value is enforced ONLY by the source pin + the behavioral test's hang-guard.]
...
- `controller/internal/controller/controller_arming_test.go` (the rung/no-collision/nil-path pins; the window twin folded to a pointer)
