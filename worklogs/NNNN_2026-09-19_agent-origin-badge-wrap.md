...
- **Root cause**: the label span carried Tailwind `truncate` (nowrap + hidden + ellipsis) — a 30-char `ses_` ID inside `inline-flex max-w-full` ellipsized.
...
## Review Round 5 (record integrity — the correction of record, completed)
...
## Review Round 7 (attribution)

The r6 correction misattributed the 7/7 to the commit message and thread comment — both verifiably never said it; the error lived in this worklog's authoritative line alone. Reworded. No code change.
