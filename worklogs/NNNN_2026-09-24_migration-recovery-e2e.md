...
**Gate GREEN — twice:** run 36077812346 on `fd1990a5` (the first green in branch history) and again on `17483cff`. The OK line at head: the api-leader-election AND api-platform-info bindings live at install+0 and +40s; the API SA can get leases and list deployments. The 39 "forbidden" grep hits in the first green's log were the assert's own echoed script text, not pod denials.
