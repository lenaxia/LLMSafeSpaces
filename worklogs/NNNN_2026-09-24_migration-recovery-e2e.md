...
**Gate GREEN — twice:** run 36077812346 on `fd1990a5` (the first green in branch history) and again on `17483cff` (run 36087507236). The OK line at head: the api-leader-election AND api-platform-info bindings live at install+0 and +40s; the API SA can get leases and list deployments. Forbidden-hit log audit, all FOUR greens (`gh run view <id> --log | grep -ci forbidden`; primaries 36077812346/36087507236, siblings 36077809401/36087505425): **the invariant that survives every observer is ZERO actual pod-log denial lines in every run** — every grep hit is the workflow's own text (Assert 3's echoed script/header/env block + its "OK: zero forbidden lines" line, or the bindings step's echoed FAIL carrier). The raw COUNT is a gh step-resolution artifact, not behavior: Assert 3's step NAME contains "forbidden", so when gh resolves step names the step's ENTIRE output column-matches (39/run — this lane's four stable measurements of the primaries); when the column renders UNKNOWN STEP only content lines match (7/run — the siblings, and the r4/r5 audits' observations). On run 36077812346 specifically, auditors observed both values (7 and 39) — per-fetch step-name resolution, not per-run behavior. The count never was the evidence; zero denials is.
