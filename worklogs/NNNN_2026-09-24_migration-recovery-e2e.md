...
**Gate GREEN — twice:** run 36077812346 on `fd1990a5` (the first green in branch history) and again on `17483cff` (run 36087507236). The OK line at head: the api-leader-election AND api-platform-info bindings live at install+0 and +40s; the API SA can get leases and list deployments. Log audit, BOTH greens (`gh run view <id> --log | grep -ci forbidden`): **39 hits each, identical distribution** — 38 = Assert 3's own `##[group]` echoed script text + its "OK: zero forbidden lines" line, 1 = the bindings step's echoed FAIL-carrier script. Every hit is the workflow's own echoed text; **zero actual pod-log denial lines in either green**. (r4 flagged this line's original wording; the re-audit at r5's request showed the count is the SAME 39 on both runs — the earlier "7 vs 39" split did not reproduce via the same command.)
