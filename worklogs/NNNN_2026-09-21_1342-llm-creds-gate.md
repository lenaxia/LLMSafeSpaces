...
2. **Noted, not fixed**: us-63's `if: ${{ env.LLM_API_KEY != '' }}` gate reads an env var nothing populates at that scope — that suite skips even WITH creds configured. Flagged for its owner (rewiring it to the secret restores us-63 coverage alongside #1342's when creds land).
...
- Owner item: configure `secrets.LLM_API_KEY` + `vars.E2E_LLM_MODEL` to restore #1342 (and rewire us-63's gate — noted defect).
