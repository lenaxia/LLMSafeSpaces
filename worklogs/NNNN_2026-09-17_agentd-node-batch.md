...
- #1415: workflow_create description names exactly script/agent/http/condition (with data contracts, otherwise-edge rule, single-start rule, dotted-path templating) and drops the invented transform/parallel/delay/mcp_call vocabulary; trigger_create teaches the camelCase workflowId spelling and the pod-scoping rule for DAG targets; workflow_update teaches the STRINGIFIED-JSON specYaml shape and the targetWorkspaceId fireability requirement (a per-run workspace override is NOT teachable — this surface cannot send one). Claims are limited to what THIS branch ships: anchoring/YAML/schema-compile wording lands with the sibling PRs (#1421/#1424) and is re-added there. Guidance pins enforce both directions (NotContains for invented types, snake_case, and the per-run-workspace clause).
