...
## D3 follow-ups (r2, #907): the two honest gaps closed

1. **Frontend retry wired to the endpoint**: useMessageQueue.retry now
   calls POST /queue/:messageId/retry first (re-arms the SAME entry —
   attempts reset, dedupe identity and ordering kept); local re-enqueue
   is the fallback for client-only entries (err_ ids). The queue
   listing carries status/attempts/lastError/clientMessageID so server
   error entries render as retryable locally.
2. **Duplicate semantics made real + honest**: the spec's "deduped at
   render" asserted a client mechanism that was never buildable
   (opencode history cannot carry clientMessageID). Shipped instead:
   pre-redelivery transcript check — on a retry, if the session
   transcript already holds a user message with the entry's exact text
   newer than accept, the entry is treated as delivered (skips the
   redelivery that would duplicate the whole turn). Closes the dominant
   duplicate source (delivery timeout while the turn completes).
   Spec amended to state exactly this; residual rare duplicates are
   accepted and render as repeated user turns.

Tests: TestOutbox_RetrySkipsAlreadyDelivered (real adapter + fake
backend serving opencode's actual history wire shape — info.role,
epoch-millis time.created). Frontend hooks 136/136.


---

## D3 follow-ups r1 review (#960): integration coverage + lint

- QF1012 lint: fmt.Fprintf instead of Write(Sprintf).
- TestOutbox_RetryEndpoint_Integration (the review's missing case):
  full wire path — accept (202) → park terminal via MaxAttempts real
  state-machine deliveries → ListQueue shows status:"error" with the
  messageID → POST /queue/:id/retry → 204 → entry pending with attempts
  zeroed → bridge delivery completes and removes it. Plus the 404 for
  an unknown id (the frontend fallback trigger).
- TestOutbox_RetryQueueMessage_NoOutbox: 501 when unwired (deterministic
  frontend fallback).
- Full handlers suite green ×2 (~98s).

### CI retrigger note
Two heads (4c8c9fb1, d0f27431) produced zero check-runs while other
PRs' workflows fired normally; a real-file push follows to retrigger.
