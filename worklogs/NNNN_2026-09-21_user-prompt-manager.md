...
- **SDK freshness — r2 correction of a false r1 claim:** the r1 note above claimed spec+parity was the freshness bar (justified by the unimplemented generate-* targets). r2's reviewer proved that WRONG: the house SDKs are HAND-WRITTEN and every other /me surface ships typed clients in all four — the 6a14c6c1 mcp-CRUD precedent is controlling. Delivered in r2: Go (UserPromptsService + wire test), TypeScript (UserPromptsAPI + envelope-unwrap test), Python (sync + async _UserPromptsAPI + respx wire test), Java (Map-based UserPromptsService facade per the TriggersService convention + 2 mock-server tests). The generate-* placeholder claim was an equivocation; the worklog records it as such.
- **r2's smaller items:** the SELECT/WHERE shape pin restored as an anchored full-query expectation (the clause-only pin let a column-drop mutation through — mutation-verified both ways this pass); the owner-scoping header claim now backed by a real handler row (user-2 → 404/no-list-visibility); HTTP-level rename-409 (router store now enforces rename-uniqueness); frontend list-failure + delete-failure rows (plus a clearAllMocks-vs-implementations test-hygiene find).
