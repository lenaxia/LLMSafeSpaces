...
### Review round 2 (one finding — V2 repair call site unasserted)

- `TestGetHistoryV2_OrphanedRunningTool_RepairedWhenIdle` + `_LiveBusySessionUntouched` drive `NewAdapterV2 → getHistoryV2Store → repairOrphanedRunningTools`; the idle pin mutation-verified red on call-site deletion.

### Review round 3 (one finding — orphan-metric Prometheus bridge unasserted)

- `TestOrphanPartsMetric_FunnelAdvances`: a sweep driven through `recordSessionStateMetrics` advances the registered `llmsafespaces_orphan_parts_aborted_total` (mutation-verified red on bridge deletion) and re-recording does not double-count (delta discipline). `llmsafespaces_orphan_parts_aborted_total` added to `TestMetricsScrape_Completeness`'s family net.
