...
### Review rounds 1–4 (CHANGES_REQUESTED → addressed)
...
- **R4 (F1′ — reviewer-reproduced refinement of the scrub rule):** r3's `cleanedSession` tracking leaked across attempts — attempt 1's CONFIRMED delete scrubbed attempt 2's LIVE survivor when attempt 2's own cleanup failed (the honest-502 shape) and ctx died in its backoff: the exact orphan class this PR fixes, and a violation of the r3 godoc's own invariant. Final guard rule: **the ctx.Done scrub fires only when `cleanedSession == resp.SessionID`** — the comparison IS the invariant (THIS response's session was confirmed gone; a prior attempt's delete is irrelevant). Regression pin: `CtxCanceledAfterFailedCleanup_MultiAttempt` (S1 gone=true, S2 gone=false + cancel inside S2's cleanup → the return keeps ses_s2); the reviewer mutation-verified it against the r3 guard.
...
- **Unknown-outcome deletes (review-adjudicated residual):** when a cleanup DELETE's response is lost after agentd executed it, the id is kept on a presumption — the engine may record a session that no longer exists. The narrow mirror of confirmed-gone semantics; accepted (r4/r5 adjudication) as the fail-safe direction — presuming existence keeps survivors discoverable, presuming deletion orphans them.
