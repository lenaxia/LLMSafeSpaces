...
- Task 2 (transport-error/script_timeout blindness): proposal sent to the orchestrator — **Option A ruled in** (agentd maps the AGENT-NODE timeout leg from 504 to 200+errorCode "script_timeout"; consistent with the established envelope pattern, zero executor changes, #1471 recording and #1476 cleanup compose immediately, classifier untouched). **Scope guardrail honored:** the script/http-node timeout legs stay 504 (their engine consumer — the pre-script branch — has no ErrorCode handling; a 200 envelope there would read as success-with-empty-output). Folded into this PR: the agentd timeout mapping, the two timeout-leg pins updated to the 200 envelope, a new `TestExecScriptNode_TimeoutLeg_Stays504` guardrail pin, and an engine composition pin (`TimeoutEnvelope_RecordsSurvivingSession_Composition`: single dispatch — non-retryable — failed fire, surviving session recorded). Future work noted in the PR: making script-node timeouts consistent requires an engine pre-script ErrorCode branch first (separate lane).
...
- `go test -count=1 -run "TestExecuteWithRetry_|TestExecuteRoutine_RetryIntermediate" ./api/internal/workflows/` — ok (7 new, red-first: the signature change failed compile pre-implementation)
...
- Task 2 proposal (timeout-leg envelope mapping — Option 2 recommended: agentd answers `script_timeout` as 200+errorCode like every other agent failure, making the executor's existing 200-parse path surface SessionID to the ErrorCode branch with zero executor changes) awaits the orchestrator's ruling.
...
- `api/internal/workflows/engine_retry_cleanup_test.go` — new (8 tests + sequenceAgentd executor)
