...
## Work Completed
- Review r1: the workflow target key now folds case-insensitively too — the reviewer demonstrated (real handler chain, 200 + cross-workspace target stored) that {"WorkflowId": ...} bound the DTO case-insensitively while escaping the exact-spelling gate. Every case variant of workflowId folds onto the canonical key (agreeable duplicates fold, contradictory variants 400), JSON null is refused explicitly (the delegated DTO binds null as preserve, contradicting the "" clear path — silently doing either was the bug), and the create path now gates case variants instead of 400ing with the misleading both-set message.
...
- Tests: case-variant gating matrix (unit + full delegated chain 403/400), create-path case-variant 201, contradictory-duplicate 400, null 400. `TestScopeTriggerUpdateBody` matrix (verbatim passthrough, case-variant retarget forced, gated DAG retarget, clear-to-routine-here, null/non-string guards) + full delegated-chain rows: workspace retarget silently forced back, workflow-target gating 403/404/200 through the REAL update handler, clear lands the routine in this workspace.
