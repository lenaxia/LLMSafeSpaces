...
6. **The worklog's "all ten cited PR numbers resolve via gh pr view" did not hold — the correction, without a mechanism overclaim this time:** #1465 is the open sentinel ISSUE (its merged fix is PR #1469); a `gh pr view` reference check is the wrong instrument for issue numbers. An earlier draft of this correction asserted the command "succeeds on issues via GraphQL shell" — observed true on this pod's gh/token, but the r2 reviewer observed the same command exit 1 with a GraphQL error in their environment: the behavior is environment-dependent (gh version/auth), so no absolute mechanism claim is safe. What is certain and load-bearing: the original worklog line claimed a verification whose subject (#1465) was not a PR in any environment. Superseding every earlier phrasing: **treat #1465 with `gh issue view`, #1469 with `gh pr view` — each verified so in the final tree.**
