...
### Review round 7 (CHANGES_REQUESTED → addressed)

Part C's root clause was false through six rounds: "sidecar mode: the file lives in the sidecar's uid-2000 space" — init-fs installs the 0400 admin-token file in BOTH modes, uid 1000, unconditioned (init_fs.go:209-214 + platform_init.go:62/69 + pod_builder.go:198), stated verbatim by agentd_sidecar.go:23-24, which is exactly WHY the sidecar consumer gets the env form (uid 2000 cannot read a 0400 uid-1000 file). The mode-split disposition: single-container → file-only (feasible, same uid); sidecar → env-only for the sidecar consumer is the existing D1-correct design — the earlier "file form unconditionally" withdrawn as infeasible for that consumer. The C3 residual is the 0400 file in BOTH modes, now declared in both §6.4b mode rows and A8's P3 set. Plus: §6.2's payload pin scoped to the C2 canary (the C4 env-secret canary rides the payload BY DESIGN — the unscoped assertion would red every sidecar run); the opencodeServeCmd citation bounds corrected (529-546).
