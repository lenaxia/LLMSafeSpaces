...
- **Finding 2 (overstated soundness comment): corrected.** The denyRoot-extends-litPrefix branch is a live overlap OR a conservative OVER-DROP ("/et?" matches only the bare "/etc" — ambient, ungoverned) — safe direction, never an under-drop; the comment and the predicate pin's why-text now say exactly that instead of "provably a live overlap in every such case".
