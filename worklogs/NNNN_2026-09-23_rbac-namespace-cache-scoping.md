...
3. **The split-namespace caveat (docs-only per the ruling):** when `api.config.kubernetes.namespace` ≠ the release namespace, the derived single-namespace cache leaves the free-models refresher's release-ns ConfigMap reads unreachable — the catalog stops refreshing with per-tick NotFound noise (`refresher.go:139-163`, non-fatal). Documented in values.yaml, helm-values.md, and the runbook entry with the workaround (`watchNamespaces="release-ns,workspace-ns"`); the watch-both code change is flagged as an owner follow-up in the PR body, NOT built here.
