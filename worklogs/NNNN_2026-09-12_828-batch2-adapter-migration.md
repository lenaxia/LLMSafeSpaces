...
- **r16 (post-merge):** the inbox merge (#1355) added two nil-check sites — the field doc's enumeration now names the inbox wiring (tryLateAnswer early-out + askLivenessOf); app.go's pre-US-65.4-done comment re-scoped to the post-batch-2 reality (Rule 5, per the admin_session precedent). Merge fidelity verified by the reviewer (all 16 batch-2 files byte-identical to the approved head; inbox interaction checked — no unguarded deref).
