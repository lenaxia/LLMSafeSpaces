...
|-------|------|---------------|--------|---------|
| opencode (#1330) | #1330: digest-pin every Dockerfile base image (docker.io availability + supply-chain gap); pin-enforcement test; README-LLM posture made true; renovate digest-pinning check | `api/Dockerfile`, `controller/Dockerfile`, `cmd/relay-proxy/Dockerfile`, `cmd/relay-router/Dockerfile`, `cmd/workspace-agentd/Dockerfile`, `runtimes/base/Dockerfile`, `runtimes/opencode/Dockerfile`, `frontend/Dockerfile`, `renovate.json`, own worklogs | DONE — 13 FROMs pinned (issue's 9 + sweep-found distroless ×4); repolint test red→green; renovate digest rule extended; PR opening | 2026-09-18 |
