...
|-------|------|---------------|--------|---------|
| opencode (#1330) | #1330: digest-pin every Dockerfile base image (docker.io availability + supply-chain gap); pin-enforcement test; README-LLM posture made true; renovate digest-pinning check | `api/Dockerfile`, `controller/Dockerfile`, `cmd/relay-proxy/Dockerfile`, `cmd/relay-router/Dockerfile`, `cmd/workspace-agentd/Dockerfile`, `runtimes/base/Dockerfile`, `runtimes/opencode/Dockerfile`, `frontend/Dockerfile`, `renovate.json`, own worklogs | In review — PR #1447 r6: 13 FROMs pinned, 6 syntax directives removed, directive matcher covers # + // forms (JSON form documented out), 29-case parser test (r5 count record corrected); awaiting re-review | 2026-09-18 |
