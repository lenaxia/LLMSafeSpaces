#!/usr/bin/env python3
# Copyright (C) 2026 Michael Kao
# SPDX-License-Identifier: AGPL-3.0-or-later
"""D-SESSION-MSG canary — Python SDK"""

from __future__ import annotations
import sys, os, time

sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), ".."))
from canary import (
    Runner,
    Config,
    config_from_env,
    wait_active,
    ensure_session_with_retry,
)
from llmsafespaces import LLMSafeSpaces
from llmsafespaces.client import message_text


def run(r: Runner, cfg: Config) -> None:
    if not cfg.llm_api_key:
        r.ok("session-msg: skipped (no LLM API key)")
        return
    c = LLMSafeSpaces(cfg.api_url, api_key=cfg.api_key, timeout=120.0)
    ws_id = None
    try:
        ok, ws = r.assert_no_error(
            lambda: c.workspaces.create(
                name="canary-py-sess-msg", runtime="base", storage_size="1Gi"
            ),
            "create: no error",
        )
        if not ok:
            return
        ws_id = ws.id

        phase = wait_active(c, ws_id)
        r.assert_(phase == "Active", "reach-active", f"got {phase!r}")
        if phase != "Active":
            return

        try:
            sess = ensure_session_with_retry(c, ws_id, 5)
        except Exception as e:
            r.fail("ensure-session: no error", str(e))
            return
        r.ok("ensure-session: no error")
        sid = sess.sessionId

        # P1–P3: Send message
        ok2, msg = r.assert_no_error(
            lambda: c.sessions.send_message(ws_id, sid, "Reply with exactly: PONG"),
            "send-message: no error",
        )
        if ok2 and msg is not None:
            r.assert_(len(message_text(msg)) > 0, "send-message: non-empty text")

        # P4: lastActivityAt updated
        ok3, st = r.assert_no_error(
            lambda: c.workspaces.get_status(ws_id), "get-status-after-msg: no error"
        )
        if ok3 and st is not None:
            r.assert_(
                st.get("lastActivityAt") is not None,
                "status: lastActivityAt non-nil after message",
            )

        # N1: nonexistent session
        r.assert_error(
            lambda: c.sessions.send_message(ws_id, "ses_nonexistent000000", "ping"),
            "send-nonexistent-session: error",
        )

    finally:
        if ws_id:
            try:
                c.workspaces.delete(ws_id)
            except:
                pass


if __name__ == "__main__":
    r = Runner("session-msg")
    cfg = config_from_env()
    run(r, cfg)
    r.print()
    sys.exit(r.exit_code())
