...
- Workspaces can be suspended (pod deleted, PVC retained) and resumed (~13–19s measured on the S5 kind suite with BOTH overlay images cold-pulled from the node — run 33402474321; the pre-overlay ~22s figure is superseded)
...
Suspend deletes the pod but retains the PVC. Activating a suspended workspace re-creates the pod (~13–19s measured with both overlay images evicted from the node — S5.5; node-cached resumes are faster still. PVC re-attach + opencode boot dominate the remaining cost). Session history in the PVC survives.
...
| `/sandbox-runtime` | emptyDir (memory, 96Mi, RW) | No — ephemeral per pod, wiped on death | agent-config.json, secrets-env, `secrets-env.rev` (batch revision anchor + apply-guard seq marker, US-70.2), `admin-prompt.md` (merged platform/org/role/user system prompt, #483), `staged-secret-files/` (R2b #1165: canonical bytes + manifest staged by the materializer, served at `GET /v1/spawn-files`), `spawn-files-ledger.json` (R2b delivery ledger — reset deletes ledger−manifest, never directories), symlink targets for SSH/git/secrets/auth.json — all file-class targets WRITTEN BY the uid-1000 consumer (ownership by construction) |
...
| `/agentd-secrets` | emptyDir (memory, 16Mi) — **sidecar mode only, never mounted in the workspace container** | No | `secrets-env`, `admin-prompt.md` — absent from uid-1000 space by mount topology (US-4b); the env delta crosses to the child via the supervisor's spawn-time PULL from `/v1/spawn-env` on the sidecar user mux (US-70.1, design 0057 — bounded wait + last-good cache, `spawned_rev` terminal verification, `degraded:spawn_env_unavailable` surfaced healthz → CRD `secretsDelivery`) |
...
**Secret batch revisions (US-70.2, design 0057 R1):** exactly one server-side builder (`SecretService.BuildWorkspaceBatch` — session identity absent from batch construction; user entries decrypt via `GetDEKServerSide`, loud per-entry degrade otherwise) produces every batch, stamped with a two-tier revision: `manifest_rev = (seq, manifestHash)` — a per-workspace monotonic counter minted by the `workspace_secret_revisions` row (DB as single writer; replicas never mint) over the decrypt-free tier (binding refs + per-secret **version counters**; `user_secrets`/`provider_credentials`/`mcp_servers` carry value-version columns, bumped on every value-affecting update) — and `batchHash` over the canonical value-bearing bytes (transport integrity + conditional pulls). Entry contract (W12): `(secretId, version, type, name, value)` + stored-only metadata — no timestamps, no derived content. `POST /internal/v1/pod-bootstrap` negotiates `contractVersion: 2`: unchanged `clientManifestHash` → **304 + ETag `<seq>:<manifestHash>`** (zero decrypts); changed → 200 envelope `{entries, revision}`; legacy pods keep receiving today's bare-array response (W15 mixed-fleet pin), and the live-push body stays legacy until demolition. Pods anchor the served revision (`secrets-env.rev` + the staged manifest), apply-guard on `seq` (stale/out-of-order pulls never downgrade; a push overlay drops the anchor back to legacy semantics), and surface `spawned_rev`/`files_rev` as `seq:manifestHash:contentHash` — terminal verification stays self-computed over what the child actually spawned with / what files exist (I4). The reconcile loop that consumes these seqs lands with US-70.3.
...
2. **Relay-only delivery hardening (design 0061, HOLD — #1548/#1546).** Four mechanisms, no more: (M1) crash-loud arming — enabled-but-unarmable staging exits the controller with code 85 within the 30s startup-guard window (the enable line IS the armed contract; extends the 81/82-agentd, 83/84-opencode ladder); (M2) migration-mode fail-open fallback — absent/expired handoff at batch time delivers the pre-flip raw-key entry + increments `relay_fallback_deliveries_total{workspace,provider_slug}` (the counter IS the stall detector; strict mode = one bool after 7 zero-days); (M3) the posture gate — ONE CI job cold-installing the chart's own shipped defaults (namespace scope, relay-only on, delivery pins) on kind, asserting all-Ready + the armed line + zero forbidden log lines + image-commit==tag-commit; carries the #1546 RBAC fixes (the split keypair rule, the pins list/watch, the flag↔RBAC key-coupling pin, the always-created inferencerelays ClusterRole); (M4) `CredentialsStaged=False` fed by the batch degrade (the one preference item). Rejected with reasons in §9: heartbeat derivation/auto-follow, staleness gauges, separate provenance jobs, flip-manifest systems — detectors ride on work that already happens.
3. **Relay injector re-arm — fetch-failure class resolved (#910), credential-change class open.** The injector's boot window is no longer one-shot-per-pod: on a terminal fetch failure (`fetch_failed`, `no_free_models`, `unhealthy_timeout`, config write failures) the SAME attempt re-arms on a generalized bounded-backoff loop (`cmd/workspace-agentd/rearm_loop.go`: waits double from 5m, capped at 30m; gated on no busy sessions via the SSE session tracker; not stacked behind a session-aware deferred restart — a `deferredRestarts` gauge is checked at cycle entry AND immediately before the relay kill, where a deferral that appeared mid-attempt skips the kill entirely because the outstanding deferred restart reads the just-written config; `HasRelay()` re-checked at each cycle entry; per-cycle `llmsafespaces_rearm_outcome_total{loop="relay_injector",outcome}`). The 2026-08-16 incident class (workspace 946a442f: boot-window fetch EOF → relay-only default unresolvable for the pod's entire lifetime) now recovers without pod recreation. US-72.4's relay-only liveness consumes the same loop. Remaining gap: the loop only starts after a retryable failure — a personal-key boot (`skipped_personal_key`, non-retryable by design: relay is correctly bypassed) never re-evaluates if the credential later changes public. That trigger still needs an explicit re-trigger.

4. **In-memory model cache is per-API-replica.** `SetModel` evicts on the replica that handled the request; other replicas serve stale data for up to 5 seconds. Future: Redis-backed cache for cross-replica consistency (US-30.11).

5. **`resolveModelWithProvider` non-determinism on collision.** When two providers in `agent-config.json` share a model ID, Go map iteration is non-deterministic — `resolveModelWithProvider` returns whichever provider the runtime visits first. In practice, provider model IDs are namespaced and do not collide, but this is not enforced.

6. **Text-only models wedge on image-bearing history (issue #1307 — platform-contained, provider not).** When a tool result carries an image into the session history (the `read` tool inlining a PNG screenshot), a text-only provider (litellm `supports_vision: false`, e.g. zai's GLM 4.x/5.x) rejects the replayed history with a 400 on every subsequent turn — the harness replays its durable store verbatim. The platform now (a) surfaces per-model `supportsVision` in the catalog (`GET /workspaces/:id/models`; the picker warns on `false`), (b) classifies the wedge 400 (`agent.ErrImageInTextOnlyHistory` → `422 {"code":"text_only_model_image_history"}` naming the recovery), and (c) repairs the SERVED transcript at read time (image parts downgraded to an explicit omission notice when the session's model is known text-only — strict failure semantics per the #1374 pattern; unknown capability never strips, the fail-safe direction). The provider-side replay itself is only fixable at the litellm boundary (pre-call hook in the external talos-ops-prod config) — filed out-of-repo. Recovery for a wedged session without any of the above: switch the model (the wedge is only fatal while pinned to the text-only model). Manual last-resort surgery runbook details worth keeping: the image exists in BOTH the opencode event payload and the `session_message.data` materialized copy in `/workspace/.local/opencode/opencode.db` (the agent serves from the materialized view, not the event store), and restart with `pkill -9 -x opencode` (`-f "opencode serve"` matches its own exec shell).
...
User-DEK credentials (env-secrets like `GH_TOKEN`, SSH keys, user LLM providers) survive a main-container restart (OOM, panic, kubelet restart) because the resync pull persists every applied envelope to the batch file on `/sandbox-cfg` (emptyDir — survives container restart, wiped on pod death): the next boot's materialize re-applies that file. The reload-cache replay that used to provide this (#443) was demolished in US-70.5.
...
- **D1 — agentd user mux, port 4097 (not admin 4098).** `PUT /v1/files` follows the validated resync-secrets pattern: the user mux (`agentd.AgentdPort = 4097`), Basic auth via `checkBasicAuthAny`. **Sidecar mode (design 0060):** uploads DELIVER in both modes — the sidecar stages upload bytes on the budgeted shared tmpfs (two-clause admission: an explicit upload budget + a credential floor against `statfs.f_bavail`) and signals the supervisor, whose `upload_apply` writes the PVC (statfs write-time gate, sha256-verified, uid-1000 ownership by construction). The pre-0060 read-only-mount clean-fail is retired.
...
- `KeyService.GetCachedDEKForUser` iterates `ListActiveJWTSessionsForUser` — it requires **at least one active (unexpired) session row** with a warm Redis cache entry, and never unwraps (US-70.5).
...
- **(a) Session-triggered only (recommended first step):** restrict v1 consumers to tasks that fire from session activity. Workspace naming qualifies — it triggers while the user is chatting, so `GetCachedDEKForUser` suffices. No new decrypt surface.
...
| Sessionless decrypt (if scope b) | `pkg/secrets/key_service.go` (`GetDEK` cache-only; `GetCachedDEKForUser` warm-cache walk) |
...
- **Workspace bindings** (3 routes): set/get bindings, reload-secrets (API-side manual resync trigger)
...
|---------|------|---------|
| 1.31 | 2026-09-21 | Design 0060 (sidecar upload delivery) retired the Epic 68 D1 sidecar clean-fail caveat: uploads DELIVER in both modes via stage-and-signal (the sidecar stages on the budgeted shared tmpfs; the supervisor's upload_apply writes the PVC). README §File Attachments D1 as-built and the epic-68 deviation updated; the e2e rows run unmodified in both modes. |
