...
if (( SCALE > 0 )); then
    ...
    while [[ $settle -lt 120 ]]; do
        ...
        for ws in "${WSBATCH[@]}"; do
            ...
            e="${e%%:*}"
            if [[ "$p" != "Active" || -z "$e" ]]; then stragglers=$((stragglers+1)); fi
            EPOCHS["$e"]=1
        done
...
for _i in $(seq 1 40); do
    ...
    # match the prefix, not a hardcoded suffix (found on the first pass:
    # grep'd id_ed25519_deploy while the file was id_ed25519_e2e-sd-ssh-deploy).
    KEYFILE=$(echo "$OUT" | grep -oE 'id_ed25519_[A-Za-z0-9._-]+' | head -1)
    if [[ -n "${KEYFILE}" ]] \
        && ! echo "$OUT" | grep -qE " (1|2000) .*id_ed25519_"; then
