...
| `controller.watchNamespaces` | string | `""` | Comma-separated namespaces to watch. `""` under `rbac.scope=cluster` = cluster-wide; under `rbac.scope=namespace` (the default) it DERIVES the workspace namespace (a cluster-wide cache cannot sync informers against namespaced RBAC — run 35872827066). `"*"` is valid ONLY under cluster scope — under namespace scope the render FAILS loudly (watch-all is incoherent there; set explicit namespaces or keep cluster scope). Split-namespace topologies (`api.config.kubernetes.namespace` ≠ release ns) should list BOTH namespaces — the derived single-namespace cache otherwise stops the free-models refresher's release-ns reads (non-fatal). |
