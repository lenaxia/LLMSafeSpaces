...
| `controller.watchNamespaces` | string | `""` | Comma-separated namespaces to watch. `""` under `rbac.scope=cluster` = cluster-wide; under `rbac.scope=namespace` (the default) it DERIVES the workspace namespace (a cluster-wide cache cannot sync informers against namespaced RBAC — run 35872827066). Any value containing `"*"` fails the render under namespace scope (watch-all or a bogus `"*"` namespace — both crashloop there; valid under cluster scope only). Split-namespace topologies (`api.config.kubernetes.namespace` ≠ release ns): the derived cache leaves the free-models refresher's catalog unreachable (no ConfigMap at all, non-fatal) — listing BOTH namespaces remedies it ONLY under cluster scope; under namespace scope the release-ns Role cannot cover a release-ns informer (same crashloop). |
...
| `rbac.scope` | string | `"namespace"` | **`namespace`** (default, G5): WRITES namespace-bound — the workspace-lifecycle Role binds in the WORKSPACE namespace (the `api.config.kubernetes.namespace` override, else the release namespace). **`cluster`**: adds the gated ClusterRole + binding — full CRUD cluster-wide on the `llmsafespaces.dev` CR families, cluster-wide READS on the core resource families its rules list (writes on those stay namespace-bound), plus conditional branches that differ — RELAY extends reads AND writes cluster-wide, REFRESHER (default-active) extends configmap READS only (its writes ride the release-ns Role); `rbac.yaml`'s rules blocks are the authoritative enumeration. (The read-only `storageclasses` ClusterRole is always created regardless of scope.) Under `namespace` scope, `controller.inferenceRelay.enabled` additionally requires `controller.watchNamespaces` (install-time guard). Even in cluster mode, no mutating verbs on secrets/pods in the base (non-conditional) rules (chart_test.go:1796-1826). |
