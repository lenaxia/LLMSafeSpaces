...
**Group entries must be public CIDRs — guarded at render time (well-formed input).** The groups render as plain `ipBlock` rules: Kubernetes rejects `except:` entries that are not subnets of the `cidr`, so the shared `blockedEgressCIDRs` subtraction cannot be attached to narrow CIDRs (it is attached only to the `0.0.0.0/0` catch-all in `public` mode). A well-formed private/internal dotted-quad entry in a group (or a narrow private `allowedEgressCIDRs` entry) **fails the render** with an error naming the entry. Scope of the guard, honestly: malformed spellings and hostnames pass the render but yield API-invalid ipBlocks that are rejected **loudly at apply time**; an aggregate CIDR spanning private space (e.g. `0.0.0.0/1`) is a residual silent case — contrived, not a plausible footgun; `extraEgressCIDRs` is unguarded by design (it exists to admit internal destinations). For in-cluster destinations, use `extraEgressCIDRs`.
