...
| K14 | Mise-shims class (2026-08-27), regression form: on a FRESH PVC (build-time shims shadowed by the mount), `kubectl exec … -c workspace -- sh -c 'command -v go && command -v python3 && command -v node'` succeeds — toolchains resolve in a non-interactive shell because the supervisor rebuilt shims at boot (ensureMiseShims) and the image PATH carries `$MISE_DATA_DIR/shims`. `sh -c` is the non-interactive shell of record: harness tool shells, CI-in-pod, anything that never sees `mise activate` | shims resolve |
