...
func Execute(ctx context.Context, language Language, handlerSource string, input any) (output json.RawMessage, stderr string, exitCode int, err error) {
	...
	if err := os.WriteFile(filepath.Join(tmpDir, handlerFile), []byte(handlerSource), 0644); err != nil {
		return nil, "", -1, fmt.Errorf("write handler: %w", err)
	}
	if err := os.WriteFile(filepath.Join(tmpDir, wrapperFile), []byte(wrapperSource), 0644); err != nil {
	...
	if err != nil {
		if ctx.Err() != nil {
			return nil, stderr, exitCode, fmt.Errorf("execution cancelled or timed out: %w", ctx.Err())
		}
