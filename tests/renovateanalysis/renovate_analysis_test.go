...
// the reusable lenaxia/ai-workflows renovate-analysis workflow (v0.2.15).
//
...
// workflow at a pinned v0.2.15 ref with the documented contract: four write
...
func TestCallerPinsReusableWorkflow(t *testing.T) {
	...
	pin := ref[1]
	if pin != "v0.2.15" && !regexp.MustCompile(`^[0-9a-f]{40}$`).MatchString(strings.Fields(pin)[0]) {
		t.Errorf("reusable ref must be the v0.2.15 tag or a 40-hex SHA pin, got %q", pin)
	}
